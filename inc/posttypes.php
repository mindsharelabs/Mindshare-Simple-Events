...
class mindEventsCPTS
{
  public function __construct()
  {
    ...
    add_action('add_meta_boxes', array($this, 'add_event_visibility_meta'));
    add_action('save_post', array($this, 'save_event_visibility_meta'));
  }
  ...
  static function add_event_visibility_meta() {
     add_meta_box(
        'event_members_only',
        'Members Only',
        function($post) {
            ...
            ?>
            <label>
                <input type="checkbox" name="members_only" value="1" <?php checked($value, '1'); ?> />
                This event is for members only (hide from public calendar)
            </label>
            <?php
        },
        ['events', 'sub_event'],
  ...
  static function save_event_visibility_meta($post_id) {
      ...
      if ($post_type === 'events') {
          ...
          foreach ($sub_events as $sub_event) {
              update_post_meta($sub_event->ID, '_members_only', '1');
          }
      }
  }
